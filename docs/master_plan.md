...
作成日: 2026-08-27
改訂履歴: 2026-09-21 (Claude/Copilot レビュー修正 & pytest 37件 PASS 完了)
...
## 1\. 現状サマリー（2026-09-21時点で確定している事実）

### 確定して良い（再検証不要）

- **コードベース健全性検証完了（2026-09-21）**: `./venv_wsl/bin/python -m pytest tests/ -v` にて全37件の単体・契約・統合テストが 100% PASS。
- **DR実装リファクタリング完了**: `reset()` でのモデル代入を廃止し、`step()` 毎の動的モデル生成へ移行。JITトレーサーリーク（`UnexpectedTracerError`）を完全に解消。
- **エピソード境界リセット保証**: `AutoResetWrapper` 外側に `EpisodeInfoResetWrapper` を注入し、終了スロットの独自 info（`step`, DR, 履歴等）が確実に 0 に初期化されることを検証済み。
- **方策クリップ健全化**: `mean_clip_scale` の二重適用を解消し、forward pass 経由でも bounds（≈3.0）が正しく機能することを確認済み。
- **物理接地**: `torso z=0.1773`で足裏誤差0.00004m、CoM(x,y)は支持基底内。8秒間のPD単体シミュレーションでroll/pitchが発散せず収束することを確認済み（純MuJoCo検証）  
- **Gate 0-P（物理ベースライン、非公式）**: 純MuJoCoフォールバックで10秒・seed=0、max\_roll=2.155°、max\_pitch=0.340°、両足接地率1.0、トルク飽和率0。
- **`time_out`フィールドの配線**: Brax本体の要求仕様(`state.info['time_out']`\+`done=True`)と一致していることをソースコードで確認済み  
- **スクリプト統廃合**: Gate 0 判定は `scratch/gate0_eval.py` に一本化。安全フィルタは `envs/cbf.py`、エクスポート系は `train/` に集約完了。

### 現在の最優先事項

- **本番学習の実行と収束挙動の監視**:
  - `python3 train/train_mjx.py` を実行し、上記改修（特に DR 動的生成と EpisodeInfoResetWrapper）により KL スパイクや episode_alive の低下が解消し、正常に学習が進むかを確認する。
- 評価時に observation normalizer の統計が学習時と正しく凍結・共有されているかの最終確認。
- 各STLパーツ（特にモータ・センサー・バッテリー搭載部）の実機重量計測（実機準備段階）。
